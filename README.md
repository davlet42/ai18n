...
**v0.8.1** — CLDR plural expansion after translate (missing `many`/`few`/… filled from `other`) + Android structural `check --platform android`. See [ROADMAP.md](./ROADMAP.md). Built on [`@cursor-translate/core`](https://github.com/davlet42/cursor-translate).
